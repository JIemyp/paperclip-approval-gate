...
const plugin = definePlugin({
    async setup(ctx) {
        ...
        ctx.events.on("heartbeat.run.queued", async (event) => {
            const { runId, agentId, issueId } = event.payload;
            ...
            await ctx.state.set({ scopeKind: "issue", scopeId: issueId, stateKey: PENDING_APPROVAL_KEY }, { runId, agentId, postedAt: new Date().toISOString() });
